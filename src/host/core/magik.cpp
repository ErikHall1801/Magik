...
{
    if(!container || !dcc_buffer || !dcc_buffer->data.config_host.h_albedo) set_and_return_error(MAGIK_ERROR_INVALID_POINTER);
...
{
    if(!container || !dcc_buffer) set_and_return_error(MAGIK_ERROR_INVALID_POINTER);
